...
from .const import ARTIK051_MODEL_PREFIX, DOMAIN, MAIN, UNIT_MAP
...
def _is_artik051(device: FullDevice) -> bool:
    """Check if device is an ARTIK051 model that needs WindFree AC quirks."""
    if (ocf := device.device.ocf) is not None and ocf.model_code is not None:
        return ARTIK051_MODEL_PREFIX in ocf.model_code
    return False
...
class SmartThingsAirConditioner(SmartThingsEntity, ClimateEntity):
    ...
    def _determine_preset_modes(self) -> list[str] | None:
        ...
        if self.supports_capability(Capability.CUSTOM_AIR_CONDITIONER_OPTIONAL_MODE):
            ...
            # ARTIK051 workaround: "quiet" works but isn't reported in supported modes
            if _is_artik051(self.device) and "quiet" not in modes:
                modes.append("quiet")
            return modes
        return None
